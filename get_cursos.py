...
def get_catalog_items(input_trail_names):
    ...
    while True:
        ...
        for item in data:
            if item['name'] in input_trail_names:
                catalog_items.append((item.get('channelId'), item.get
                ('contentStructureId')))
                found_trails.add(item['name'])
            
        print(f"Found trails: {found_trails}")
        print(f"Trail names: {set(input_trail_names)}")
        print(f'Processed {len(catalog_items)} items.')


        if found_trails == set(input_trail_names):
            break
        
        CATALOG_STARTS += CATALOG_LIMIT
...
def get_course_structure(structure_id, course_id, course_names, courses_status):
    ...
    structure = []
    for day in data["rootLevel"]['items']:
        if 'level' in day:
            day_name = day['level']['name']
            day_courses = [(course_names[item['itemId']], courses_status[item['itemId']])
                           for item in day['level']['items'] if item['itemId'] in course_names]
            structure.append({day_name: day_courses})

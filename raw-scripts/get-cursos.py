...
TRAIL_NAMES = ['Consultas e Ajustes - Salesforce', 'TR Gold Móvel Business']
if len(TRAIL_NAMES) > 10:
    print("Erro: o máximo de trilhas é de até 10")
    exit()
...
def get_catalog_items():
    ...
    for trail_name in TRAIL_NAMES:
        response = requests.get(
           f"https://api.prod.timetoknow.com/PlayAppService/catalogItems/v2",
           params={
...
def get_course_names(course_id):
    ...
    id_conteudo = {course['id']: course['id'] for course in data['libraryItems']}
    print(f"O ID do curso {course_names}, ID: {id_conteudo}\n")
